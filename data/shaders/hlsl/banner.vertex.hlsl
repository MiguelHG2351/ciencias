...
void main(in VS_Input VSInput, out PS_Input PSInput)
{
	...
	#ifdef USE_SKINNING
		PSInput.position = mul(WORLDVIEWPROJ_STEREO[i], mul(BONES[int(VSInput.boneId)], float4(VSInput.position, 1)));
	#else
...
#else
	#ifdef USE_SKINNING
		PSInput.position = mul(WORLDVIEWPROJ, mul(BONES[int(VSInput.boneId)], float4(VSInput.position, 1)));
	#else
...
#ifdef USE_SKINNING
	float L = lightIntensity(BONES[int(VSInput.boneId)], float4(VSInput.position, 1), normal);
	...
	PSInput.texCoords.zw = VSInput.texCoords.xy;
